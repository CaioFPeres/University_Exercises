...
void listar_transacoes_periodo(char *data_inicio, char *data_fim) {
    ...
    double inicio = atof(data_inicio);
    double fim = atof(data_fim);


    for(int i = 0; i < qtd_registros_transacoes; i++){

        double timestamp = atof(timestamp_cpf_origem_idx[i].timestamp);
...
void listar_transacoes_cpf_origem_periodo(char *cpf, char *data_inicio, char *data_fim) {
    ...
    for(int i = 0; i < qtd_registros_transacoes; i++){
        ...
        if(strcmp(timestamp_cpf_origem_idx[i].cpf_origem, cpf) == 0 && timestamp >= inicio && timestamp <= fim){
