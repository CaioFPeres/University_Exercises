    ...
    for (unsigned i = 0; i < qtd_registros_clientes; ++i) {
        ...
        for(int j = 0; j < 4; j++){
            if(c.chaves_pix[j][0] != '\0'){
                ...
                // coloca a chave pix na string
                strcpy(chavePixCPF, &c.chaves_pix[j][1]);

                for(k = 0; k < TAM_MAX_CHAVE_PIX; k++){
                    if(chavePixCPF[k] == '\0')
                        break;
                }

                //coloca os # pra ocupar o espaço vazio
                for(k = k; k < TAM_MAX_CHAVE_PIX-1; k++){
                    chavePixCPF[k] = '#';
                }
                ...
                // coloca o cpf
                strcat(chavePixCPF, c.cpf);

                //insere
                btree_insert(chavePixCPF, &chaves_pix_idx);
...
void escrever_registro_cliente(Cliente c, int rrn) {
    ...
    strncpy(&ARQUIVO_CLIENTES[rrn*TAM_REGISTRO_CLIENTE + TAM_CPF + tamNome + 1 + TAM_NASCIMENTO + tamEmail + 1 + TAM_CELULAR + TAM_SALDO], chavesPix, k);
    strncpy(&ARQUIVO_CLIENTES[rrn*TAM_REGISTRO_CLIENTE + TAM_CPF + tamNome + 1 + TAM_NASCIMENTO + tamEmail + 1 + TAM_CELULAR + TAM_SALDO + k], ";", 1);
...
void escrever_registro_transacao(Transacao t, int rrn) {
    ...
    strncpy(&ARQUIVO_CLIENTES[rrn*TAM_REGISTRO_CLIENTE], t.cpf_origem, TAM_CPF - 1);

    strncpy(&ARQUIVO_CLIENTES[rrn*TAM_REGISTRO_CLIENTE + TAM_CPF - 1], t.cpf_destino, TAM_CPF - 1);

    strncpy(&ARQUIVO_CLIENTES[rrn*TAM_REGISTRO_CLIENTE + TAM_CPF - 1 + TAM_CPF - 1], valorChar, TAM_SALDO - 1);

    strncpy(&ARQUIVO_CLIENTES[rrn*TAM_REGISTRO_CLIENTE + TAM_CPF - 1 + TAM_CPF - 1 + TAM_SALDO - 1], t.timestamp, TAM_TIMESTAMP - 1);
...
void cadastrar_cliente_menu(char *cpf, char *nome, char *nascimento, char *email, char *celular) {
    ...
    if(clientes_idx.rrn_raiz != -1 && btree_search(NULL, false, cpf, clientes_idx.rrn_raiz, &clientes_idx)){
    ...
    char cpfRrn[TAM_CHAVE_CLIENTES_IDX + 1];
    sprintf(cpfRrn, "%s%04d", c.cpf, qtd_registros_clientes);
    
    btree_insert(cpfRrn, &clientes_idx);
...
void cadastrar_chave_pix_menu(char *cpf, char tipo) {
    ...
    char chavepix[TAM_MAX_CHAVE_PIX], pix_aux[TAM_MAX_CHAVE_PIX], pixSearch[TAM_MAX_CHAVE_PIX];
    char aleatorio[37], cpfRrn[TAM_CHAVE_CLIENTES_IDX + 1];
    ...
    //btree search
    if(btree_search(cpfRrn, false, cpf, clientes_idx.rrn_raiz, &clientes_idx) == false){
    ...
    strcpy(pixSearch, &chavepix[remover]);

    for(int k = length-1; k < TAM_MAX_CHAVE_PIX; k++){
        pixSearch[k] = '#';
    }

    pixSearch[TAM_MAX_CHAVE_PIX-1] = '\0';


    if(chaves_pix_idx.rrn_raiz != -1 && btree_search(pix_aux, false, pixSearch, chaves_pix_idx.rrn_raiz, &chaves_pix_idx)){
    ...
    // coloca a chave pix na string
    strcpy(chavePixCPF, &chavepix[remover]);

    //coloca os # pra ocupar o espaço vazio
    if(length < TAM_CHAVE_CHAVES_PIX_IDX + 1){
        for(k = length-remover; k < TAM_MAX_CHAVE_PIX - 1; k++){
            chavePixCPF[k] = '#';
        }
    }
...
void transferir_menu(char *chave_pix_origem, char *chave_pix_destino, double valor) {
    ...
    char searchPixOrigem[TAM_CHAVE_CHAVES_PIX_IDX], searchPixDestino[TAM_CHAVE_CHAVES_PIX_IDX];
    char searchClienteOrigem[TAM_CHAVE_CLIENTES_IDX + 1], searchClienteDestino[TAM_CHAVE_CLIENTES_IDX + 1];
    char cpfPixOrigem[TAM_CPF], cpfPixDestino[TAM_CPF];

    char pixFormatadaOrigem[TAM_MAX_CHAVE_PIX];
    char pixFormatadaDestino[TAM_MAX_CHAVE_PIX];

    pixFormatadaOrigem[0] = '\0';
    pixFormatadaDestino[0] = '\0';
    

    //pega tamanho e copia da origem
    for(tamPixOrigem = 0; tamPixOrigem < TAM_MAX_CHAVE_PIX-1; tamPixOrigem++){
        if(chave_pix_origem[tamPixOrigem] != '\0')
            pixFormatadaOrigem[tamPixOrigem] = chave_pix_origem[tamPixOrigem];
        else
            break;
    }

    pixFormatadaOrigem[tamPixOrigem] = '\0';

    // coloca os # na chavepix origem

    for(k = tamPixOrigem; k < TAM_MAX_CHAVE_PIX-1; k++){
        pixFormatadaOrigem[k] = '#';
    }

    pixFormatadaOrigem[TAM_MAX_CHAVE_PIX-1] = '\0';
    

    //pega tamanho e copia da destino
    for(tamPixDestino = 0; tamPixDestino < TAM_MAX_CHAVE_PIX-1; tamPixDestino++){
        if(chave_pix_destino[tamPixDestino] != '\0')
            pixFormatadaDestino[tamPixDestino] = chave_pix_destino[tamPixDestino];
        else
            break;
    }

    pixFormatadaDestino[tamPixDestino] = '\0';

    //coloca os # na destino
    for(k = tamPixDestino; k < TAM_MAX_CHAVE_PIX-1; k++){
        pixFormatadaDestino[k] = '#';
    }

    pixFormatadaDestino[TAM_MAX_CHAVE_PIX-1] = '\0';



    // pesquisa na arvore de pix
    if(chaves_pix_idx.rrn_raiz != -1 && (!btree_search(searchPixOrigem, false, pixFormatadaOrigem, chaves_pix_idx.rrn_raiz, &chaves_pix_idx) || !btree_search(searchPixDestino, false, pixFormatadaDestino, chaves_pix_idx.rrn_raiz, &chaves_pix_idx)) ){
    ...
    // pega cpf do arquivo de indice de cada um
    strcpy(cpfPixOrigem, &searchPixOrigem[TAM_CHAVE_CHAVES_PIX_IDX - TAM_CPF + 1]);
    strcpy(cpfPixDestino, &searchPixDestino[TAM_CHAVE_CHAVES_PIX_IDX - TAM_CPF + 1]);


    // comparação para ver se as chaves pix pertencem ao mesmo cpf
    if(strcmp(cpfPixOrigem, cpfPixDestino) == 0){
        printf(ERRO_CHAVE_PIX_INVALIDA);
        return;
    }
    ...
    if(clientes_idx.rrn_raiz != -1 && (!btree_search(searchClienteOrigem, false, cpfPixOrigem, clientes_idx.rrn_raiz, &clientes_idx) || 
    !btree_search(searchClienteDestino, false, cpfPixDestino, clientes_idx.rrn_raiz, &clientes_idx) ) ){
    ...
    searchClienteOrigem[TAM_CHAVE_CLIENTES_IDX] = '\0';
    searchClienteDestino[TAM_CHAVE_CLIENTES_IDX] = '\0';



    //pega o rrn do arquivo de cliente
    rrnOrigem = atoi(&searchClienteOrigem[TAM_CPF-1]);
    rrnDestino = atoi(&searchClienteDestino[TAM_CPF-1]);
...
void buscar_cliente_chave_pix_menu(char *chave_pix) {
    ...
    char result[TAM_CHAVE_CHAVES_PIX_IDX + 1];
    int k = 0, tamChave = 0;
    ...
    // coloca a chave pix na string
    strcpy(chavePix, chave_pix);


    //contar tamanho da chave pix
    for(tamChave = 0; tamChave < TAM_MAX_CHAVE_PIX - 1; tamChave++){
        if(chave_pix[tamChave] == '\0')
            break;
    }

    for(k = tamChave; k < TAM_MAX_CHAVE_PIX - 1; k++){
        chavePix[k] = '#';
    }
    ...
    if(!btree_search(result, true, chavePix, chaves_pix_idx.rrn_raiz, &chaves_pix_idx)){
        printf(ERRO_REGISTRO_NAO_ENCONTRADO);
        return;
    }


    buscar_cliente_cpf_menu(&result[TAM_MAX_CHAVE_PIX - 1]);
...
void listar_clientes_cpf_menu() {
    ...
    if(!btree_print_in_order(NULL, NULL, exibir_btree_cliente, clientes_idx.rrn_raiz, &clientes_idx)){
...
bool alterar_saldoSemOK(char *cpf, double valor){
    ...
    char cpfRrn[TAM_CHAVE_CLIENTES_IDX + 1], valorChar[TAM_SALDO];
    int rrn = 0, i = 0, insereEm = 0;
    double saldo;

    //btree search
    if(clientes_idx.rrn_raiz == -1 || !btree_search(cpfRrn, false, cpf, clientes_idx.rrn_raiz, &clientes_idx)){
        printf(ERRO_REGISTRO_NAO_ENCONTRADO);
        return false;
    }
