...
void dfs_private(Graph *G, int vis[], unsigned v, int cor[]){
   vis[v] = 1;
   cor[v] = 0;
...
void dfs(Graph *G){
   ...
   for(int i = 0; i < G->V; i++){
      verifica = cor[i];
      for(Node* n = G->adj[i]; n!= NULL; n = n->next){

         if(verifica == cor[n->w]){
            printf("Nao admite biparticao ");
            return;
         }
